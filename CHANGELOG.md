...
- Multicursor (`Alt+Shift+Up`/`Alt+Shift+Down` add above/below, `Alt+D`
  ...
  typing, Backspace/Delete, Enter, Tab, autopairs and paste apply at every
  caret in a single undo step; motion moves all carets; extra carets render
  as highlighted cells. Commands are rebindable via `keybindings.json` and
